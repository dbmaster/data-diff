...
if ("File".equals(p_target)) {
    ...
    try {
        ...
        io.dbmaster.tools.diff.CompareData.executeAsync(dbm, connection1, connection2, p_source_sql, p_target_sql, p_pk, p_limit, logger, pw, p_output_filters, p_options)
        pw.flush();
    } finally { 
        if (outStream!=null) { 
            outStream.close();
